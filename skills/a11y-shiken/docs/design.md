...
**出力**: Excel（`--output`）と `merged-result.json`（axe-core JSON と同じディレクトリ）。
...
| 基準 | チェック名 | 手法 |
|------|-----------|------|
| 2.4.7 | フォーカス可視化 | Tab で移動し、フォーカス前後のスタイル差分とスクリーンショットで確認 |
| 2.4.3 | フォーカス順序 | Tab 移動順序をトレースし、DOM 順序と比較 |
| 2.1.2 | キーボードトラップ | Tab / Shift+Tab で全フォーカス可能要素を巡回し、無限ループを検出 |
